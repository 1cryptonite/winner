...
async function getActiveMatchesBySportId(req, res) {
	...
	try {
		...
		logger.errorlog.error("getActiveMatchesBySportId", e);
		return apiSuccessRes(req, res, 'not found.');
	}
...
async function getOnlineMatch(req, res) {
	...
	console.log(settings.ONLINE_MATCH_URL + sport_id + '&CompetitionID=' + series_id);
...
async function marketWatch(req, res) {
	try {
		...
		if (fancyList.statusCode === CONSTANTS.SUCCESS) {
			for (let k in fancyList.data) {
				let fData = fancyList.data[k];
				console.log(fData);
				let getMatchUserData = await matchesService.getMarketPosition(userid, fData.market_id, loginUserData.user_type_id);
				fData['position'] = getMatchUserData.data;

				reponce.push(fData);
			}
			return apiSuccessRes(req, res, 'market match get successfully', reponce);
		} else {
			return apiSuccessRes(req, res, 'Error to get marketWatch.');
		}
	} catch (e) {
		logger.errorlog.error("marketWatch", e);
	}
}
...
async function matchlist(req, res) {
	...
	try {
		...
		logger.errorlog.error("matchlist", axiosErr);
	...
	if (checkSeriesIsAdded.data.length > 0) {
		...
		if (checkMatchIsAdded.data) {
			...
			if (datafromService.statusCode === CONSTANTS.SUCCESS) {
				...
				return apiErrorRes(req, res, datafromService.statusCode);
			}
		}
	} else {
...
async function getUserBook(req, res) {
	...
	let returnData = await marketsService.getUserBook(user_id, user_type_id, market_id);
	if (returnData.statusCode === CONSTANTS.SUCCESS) {
		return apiSuccessRes(req, res, 'success', returnData.data);
	} else if (returnData.statusCode === CONSTANTS.NOT_FOUND) {
		return apiSuccessRes(req, res, 'not found.');
...
router.post('/matches/user-book', getUserBook);
