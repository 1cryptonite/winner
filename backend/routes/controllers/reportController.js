...
if (pdf_download == 1) {
	...
	globalFunction.printPDF(req, res, pdfName, tabledata, fileData);
		...
		logger.errorlog.error("profitLossMatchWise",e);
...
if (pdf_download == 1) {
	...
	let finalData = { "user_id": user_id, parent_user_type_id: getUserById.data.parent_user_type_id, "parent_id": getUserById.data.parent_id, data : profitLossData.data};
		...
		logger.errorlog.error("profitLossUpline",e);
        return apiErrorRes(req, res, 'Enter valid param!', e);
    }
}
...
async function profitLossUplineBySport(req, res) {
    ...
    try {
		...
		logger.errorlog.error("profitLossUplineBySport",e);
        return apiErrorRes(req, res, 'Enter valid param!', e);
    }
}
...
async function settlementReport(req, res) {
	...
	try {
		...
		logger.errorlog.error("settlementReport",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function makeSettlement(req, res) {
	...
	try {
		...
		let getUData = await userService.getUserById(user_id);
		let getParentData = await userService.getUserById(parent_id);

		
		if (getUData.statusCode === CONSTANTS.SUCCESS) {

			if(getUData.data.self_lock_settlement == '0' && getUData.data.parent_lock_settlement == '0') {

				if (type == 1) {
					user_comment = 'Cash recevied from ('+getParentData.data.user_name+') ';		
					parent_comment = 'Cash given to ('+getUData.data.user_name+') ';	
				//	comment = 'Cash Debit';
				} else if (type == 2) {
				//	comment = 'Cash Credit'; 
					user_comment = 'Cash given to ('+getParentData.data.user_name+') ';
					parent_comment = 'Cash recevied from ('+getUData.data.user_name+') ';	
				}
				...
				let returnData = await reportService.makeSettlement(user_id, user_type_id, parent_id, amount, type, user_comment,parent_comment);
		...
		logger.errorlog.error("makeSettlement",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function settlementCollectionHistory(req, res) {
	...
	try {
		...
		logger.errorlog.error("settlementCollectionHistory",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function deleteSettlement(req, res) {
	...
	try {
		...
		logger.errorlog.error("deleteSettlement",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function superAdminCommissionDetail(req, res) {
	try {
		...
		logger.errorlog.error("superAdminCommissionDetail",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function makeSuperAdminCommissionSettlement(req, res) {
	...
	try {
		...
		logger.errorlog.error("makeSuperAdminCommissionSettlement",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function deleteSuperAdminCommissionSettlement(req, res) {
	...
	try {
		...
		logger.errorlog.error("deleteSuperAdminCommissionSettlement",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function settlementHistoryByParent(req, res) {
	...
	try {
		...
		logger.errorlog.error("settlementHistoryByParent",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
...
async function fancyStackUserWise(req, res) {
	...
	try {
		...
		logger.errorlog.error("fancyStackUserWise",e);
		return apiErrorRes(req, res, 'Enter valid param!', e);
	}
}
