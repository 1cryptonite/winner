...
let getFancyBetForUserPosition = async (user_id,fancy_id,user_type_id=null,notInIds=[]) => {
    try {
        let condition;
        let selectioName;
        switch(user_type_id) {
...
let getRunTimeFancyPosition = async (user_id,fancy_id,user_type_id,notInArray) => {
    try {
        let data;
        let fancyList = await  getFancyBetForUserPosition(user_id,fancy_id,user_type_id,notInArray);
       let minus = 1;
        /* if(user_type_id!=5 || user_type_id==null){
             minus = 1;
        }*/
