...
let getPlayerInfoVirtualGame = async (userId) => {
	try {
		...
		logger.errorlog.error("getPlayerInfoVirtualGame",error);
...
let getBalanceVirtualGame = async (userId) => {
	try {
		...
		logger.errorlog.error("getBalanceVirtualGame",error);
		return resultdb(CONSTANTS.SERVER_ERROR, CONSTANTS.DATA_NULL);
	}
};
...
let brinoPlaceBet = async (userId, data) => {
	...
	try {
		...
		logger.errorlog.error("brinoPlaceBet",error);
		return resultdb(CONSTANTS.SERVER_ERROR, returnData);
	}
};
...
WHERE a.user_id = ? AND a.market_id = ? AND a.game_type = '1';`, [userpl, liability, userpl, userpl, liability, userId,
		...
		logger.errorlog.error("brinoResult",error);
		return returnData;
	}
};
let brinoCancelGame = async (data) => {
	...
	try {
		...
		logger.errorlog.error("brinoCancelGame",error);
		return resultdb(CONSTANTS.SERVER_ERROR, returnData);
	}
};
...
let fantasyJoinContest = async (userId, data) => {
	try {
		...
		logger.errorlog.error("fantasyJoinContest",error);
		return {
			"code": 1,
			"error": true,
			"msg": "An error occurred",
...
let fantasyResult = async (data) => {
	try {
		...
		let validateBet = await MysqlPool.query(`SELECT id, amount, IFNULL(win_amount, '') AS win_amount, IFNULL(is_rollbacked, '') AS is_rollbacked FROM brino_fantasy_contests WHERE user_id = ? AND txn_id = ? LIMIT 1;`, [userId, data.txnId]);
		if (validateBet.length > 0) {
			...
			else if (validateBet[0].is_rollbacked == '1') {
				let userData1 = await getBalanceVirtualGame(userId);
				if (userData1.statusCode === CONSTANTS.SUCCESS) {
					return {
						"code": 1,
						"error": true,
						"msg": "Contest already rollback",
						"data": {
							"amount": userData1.data.balance
						}
					};
...
WHERE a.user_id = ? AND a.market_id = ? AND a.game_type = '6';`, [data.amount, data.amount, userpl, userData[0].id,
		...
		logger.errorlog.error("fantasyResult",error);
		return {
			"code": 1,
			"error": true,
			"msg": "An error occurred",
...
let fantasyGetBalance = async (userId) => {
	try {
		let userData = await MysqlPool.query(`SELECT balance FROM users WHERE id = ? AND user_type_id = 5 LIMIT 1;`, [userId]);
		if (userData.length > 0) {
			return {
		...
		logger.errorlog.error("fantasyGetBalance",error);
		return {
			"code": 1,
			"error": true,
			"msg": "An error occurred",
...
let fantasyRollbackContest = async (data) => {
	try {
		let userId = parseInt(data.userId);
		let validateBet = await MysqlPool.query(`SELECT id, amount, IFNULL(win_amount, '') AS win_amount, IFNULL(is_rollbacked, '') AS is_rollbacked FROM brino_fantasy_contests WHERE user_id = ? AND txn_id = ? LIMIT 1;`, [userId, data.txnId]);
		if (validateBet.length > 0) {
			if (validateBet[0].win_amount != '') {
				...
				await conn.beginTransaction();
				try {
					await conn.query('UPDATE brino_fantasy_contests SET is_rollbacked = "1" WHERE id = ?', [validateBet[0].id]);
					await conn.query('UPDATE users SET balance = balance + ?, total_balance = total_balance + ? WHERE id = ?', [validateBet[0].amount, validateBet[0].amount, userId]);
					await conn.commit();
					conn.release();
					let userData1 = await getBalanceVirtualGame(userId);
					if (userData1.statusCode === CONSTANTS.SUCCESS) {
		...
		logger.errorlog.error("fantasyRollbackContest",error);
		return {
			"code": 1,
			"error": true,
			"msg": "An error occurred",
